// ⚔️ TACTICAL COMBAT SYSTEM - THE LAST COVENANT
// AAA+ Darkest Dungeon inspired Combat
// Renamed to avoid conflict with legacy CombatSystem in card-system.js

class TacticalCombatSystem {
  constructor(allies, enemies, environment = 'dungeon', onCombatEnd = null) {
    // Units
    ...
    // Callback
    this.onCombatEnd = onCombatEnd;  // ✅ Callback pour retourner au jeu principal
  ...
  async handleVictory() {
    ...
    // ✅ Appeler le callback de fin de combat après victoire
    await delay(2000);
    this.closeCombatModal();

    if (this.onCombatEnd) {
      this.onCombatEnd({ victory: true, allies: this.allies, enemies: this.enemies });
    }
  }
  ...
  async handleDefeat() {
    ...
    // ✅ Appeler le callback de fin de combat après défaite
    if (this.onCombatEnd) {
      this.onCombatEnd({ victory: false, allies: this.allies, enemies: this.enemies });
    } else if (typeof returnToCamp === 'function') {
...
console.log('⚔️ Tactical Combat System AAA+ loaded');