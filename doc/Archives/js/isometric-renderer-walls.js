/**
 * 🎨 ISOMETRIC 2.5D RENDERER - AVEC MURS ET PORTES
 * THE LAST COVENANT - Version avec Murs
 *
 * Rendu isométrique avec :
 * - Murs hauts verticaux sur bords des tuiles
 * - Portes automatiques entre tuiles connectées
 * - Tuiles qui se touchent
 * - Détection de connectivité pour les murs
 */

class IsometricRendererWalls {
  ...
  renderDungeonPath(dungeon) {
    ...
    // PASS 1 : Dessiner tous les SOLS d'abord
    dungeon.path.forEach((tile, index) => {
      this.renderFloor(tile, index);
    });

    // PASS 2 : Dessiner tous les MURS après (avec tri par profondeur)
    // Trier les tuiles par profondeur ISO (y) - de l'arrière vers l'avant
    const sortedIndices = dungeon.path.map((tile, index) => ({
      index,
      depth: this.organicPath[index].y + this.organicPath[index].x // Profondeur ISO
    })).sort((a, b) => a.depth - b.depth); // Du plus loin au plus proche

    sortedIndices.forEach(({ index }) => {
      this.renderWalls(dungeon.path[index], index);
    });
  }

  /**
   * Dessine uniquement le SOL d'une tuile
   */
  renderFloor(tile, index) {
    const cart = this.organicPath[index];

    // Calculer les 4 COINS du carré cartésien en coordonnées ISO
    const corner1 = this.cartesianToIso(cart.x, cart.y);
    const corner2 = this.cartesianToIso(cart.x + 1, cart.y);
    const corner3 = this.cartesianToIso(cart.x + 1, cart.y + 1);
    const corner4 = this.cartesianToIso(cart.x, cart.y + 1);
    ...
    // Dessiner le SOL (losange)
    this.ctx.fillStyle = '#2a2832';
    this.ctx.beginPath();
  ...
  /**
   * Vérifie si une tuile voisine existe à une position cartésienne donnée
   */
  hasNeighbor(cartX, cartY) {
    return this.organicPath.some(tile => tile.x === cartX && tile.y === cartY);
  }

  /**
   * Dessine les MURS pour une tuile
   * Murs SEULEMENT sur les bords SANS voisin (= extérieur du couloir)
   * Pas de mur = PASSAGE entre tuiles adjacentes !
   */
  renderWalls(tile, index) {
    const cart = this.organicPath[index];

    // Vérifier les 4 voisins
    const hasNorth = this.hasNeighbor(cart.x, cart.y - 1);
    const hasEast = this.hasNeighbor(cart.x + 1, cart.y);
    const hasSouth = this.hasNeighbor(cart.x, cart.y + 1);
    const hasWest = this.hasNeighbor(cart.x - 1, cart.y);

    // Calculer les 4 COINS du carré cartésien
    const corner1 = this.cartesianToIso(cart.x, cart.y);
    const corner2 = this.cartesianToIso(cart.x + 1, cart.y);
    const corner3 = this.cartesianToIso(cart.x + 1, cart.y + 1);
    const corner4 = this.cartesianToIso(cart.x, cart.y + 1);
    ...
    // MUR NORD-EST (segment NW→NE) si PAS de voisin NORD
    if (!hasNorth) {
      this.ctx.fillStyle = '#1a1820';
      this.ctx.beginPath();
      this.ctx.moveTo(screenNW.x, screenNW.y);
      this.ctx.lineTo(screenNW.x, screenNW.y - wallHeight);
      this.ctx.lineTo(screenNE.x, screenNE.y - wallHeight);
      this.ctx.lineTo(screenNE.x, screenNE.y);
      this.ctx.closePath();
      this.ctx.fill();
    ...
    // MUR SUD-EST (segment NE→SE) si PAS de voisin EST
    if (!hasEast) {
      this.ctx.fillStyle = '#12111a';
      this.ctx.beginPath();
      this.ctx.moveTo(screenNE.x, screenNE.y);
      this.ctx.lineTo(screenNE.x, screenNE.y - wallHeight);
      this.ctx.lineTo(screenSE.x, screenSE.y - wallHeight);
      this.ctx.lineTo(screenSE.x, screenSE.y);
      this.ctx.closePath();
      this.ctx.fill();
    ...
    // MUR SUD-OUEST (segment SE→SW) si PAS de voisin SUD
    if (!hasSouth) {
      this.ctx.fillStyle = '#1a1820';
      this.ctx.beginPath();
      this.ctx.moveTo(screenSE.x, screenSE.y);
      this.ctx.lineTo(screenSE.x, screenSE.y - wallHeight);
      this.ctx.lineTo(screenSW.x, screenSW.y - wallHeight);
      this.ctx.lineTo(screenSW.x, screenSW.y);
      this.ctx.closePath();
      this.ctx.fill();
    ...
    // MUR NORD-OUEST (segment SW→NW) si PAS de voisin OUEST
    if (!hasWest) {
      this.ctx.fillStyle = '#12111a';
      this.ctx.beginPath();
      this.ctx.moveTo(screenSW.x, screenSW.y);
      this.ctx.lineTo(screenSW.x, screenSW.y - wallHeight);
      this.ctx.lineTo(screenNW.x, screenNW.y - wallHeight);
      this.ctx.lineTo(screenNW.x, screenNW.y);
      this.ctx.closePath();
      this.ctx.fill();
...
// Export (exposer comme IsometricRenderer pour compatibilité avec game-test.js)
window.IsometricRenderer = IsometricRendererWalls;

console.log('✅ IsometricRendererWalls (avec Murs et Portes) chargé');
